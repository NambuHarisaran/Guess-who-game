...
/**
 * Helper function to read games from JSON file
...
/**
 * Helper function to write games to JSON file
...
exports.updateGame = (req, res) => {
    try {
        ...
        if (revealedTiles !== undefined) {
            games[gameIndex].revealedTiles = JSON.parse(revealedTiles);
        }
        
        // Update image if new one uploaded
        if (req.file) {
            // Delete old image
            const oldImagePath = path.join(__dirname, '../../', games[gameIndex].image);
            if (fs.existsSync(oldImagePath)) {
                fs.unlinkSync(oldImagePath);
            }
            games[gameIndex].image = `/uploads/${req.file.filename}`;
        }
...
exports.deleteGame = (req, res) => {
    try {
        ...
        const imagePath = path.join(__dirname, '../../', games[gameIndex].image);
